...
def _write_training_time_file(ckpt_dir, fallback_dir, total_seconds, status, completed_epochs, resumed):
    target_dir = ckpt_dir if ckpt_dir is not None else fallback_dir
    training_time_path = os.path.join(target_dir, "training_time.txt")
    with open(training_time_path, "w") as outfile:
        outfile.write("status: {}\n".format(status))
        outfile.write("resumed: {}\n".format(resumed))
        outfile.write("completed_epochs: {}\n".format(completed_epochs))
        outfile.write("total_training_time_seconds: {:.2f}\n".format(total_seconds))
        outfile.write("total_training_time_hms: {}\n".format(_format_duration(total_seconds)))
...
def train(config, device, resume=False):
    ...
    try:
        ...
        if config.train.hdf5_normalize_obs:
            obs_normalization_stats = getattr(trainset, "get_obs_normalization_stats")()
        ...
        if resume:
            ...
            previous_training_time = variable_state.get("total_training_time", 0.0)
            print("*" * 50)
        ...
        for epoch in range(start_epoch, config.train.num_epochs + 1):
            ...
            total_training_time = previous_training_time + (time.time() - train_start_time)
            ...
            if should_save_ckpt:    
                ...
                TrainUtils.save_model(
                    ...
                    ckpt_path=os.path.join(ckpt_dir, "{}.pth".format(epoch_ckpt_name)),
                    obs_normalization_stats=obs_normalization_stats,
                    action_normalization_stats=action_normalization_stats,
                )
            ...
            _write_training_time_file(
                ckpt_dir=ckpt_dir,
                fallback_dir=time_dir,
        ...
        total_training_time = previous_training_time + (time.time() - train_start_time)
        training_time_path = _write_training_time_file(
            ckpt_dir=ckpt_dir,
            fallback_dir=time_dir,
            total_seconds=total_training_time,
        ...
        print("\nTotal training time: {} ({:.2f}s)".format(_format_duration(total_training_time), total_training_time))
        print("Training time written to {}\n".format(training_time_path))
